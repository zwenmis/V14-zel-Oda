...
if(value == "oda-oluştur"){
    ...
    craxModal.addComponents(name,limit);

await button.showModal(craxModal);
...
const craxModal = new ModalBuilder()
    ...
    .setCustomId('kisi')
    .setPlaceholder(`örn; Kullanıcı ID`)
    ...
    craxModal.addComponents(kisirow);
...
Odaya Giriş İzni Olan Kullanıcılar; ${users ? users.map((crax,n) => `${n+1}).${button.guild.members.cache.get(crax).user.tag}`).join(", ") : "Bulunamadı"}
\`\`\``,ephemeral:true})
...
button.guild.channels.create({
        ...
        let invite = await crax.createInvite({maxUses: 1});
        ...
        await client.db.set(`özeloda_${button.member.id}`,`${crax.id}`)
        await client.db.set(`${crax.id}`,`${button.member.id}`)
        await client.db.push(`members_${crax.id}`,button.member.id)
        })
    ...
    let channel = button.guild.channels.cache.get(data);
    button.guild.members.fetch(kisi).then(crax => {
    if (crax.voice.channel.id !== channel.id) return button.reply({content:`> **Belirtilen Üye Özel Oda Kanalında Bulunmamakta!**`, ephemeral:true })
    button.reply({content:`> **${crax} Ses Kanalından Başarıyla Atıldı!**`, ephemeral: true })
    crax.voice.disconnect()
